        var taskStore = new Ext.ux.maximgb.tg.AdjacencyListStore({
            ...
            proxy: new Ext.data.HttpProxy({
                url: 'lib/ext-gantt/tasks.xml',
                //url : 'lib/ext-gantt/tasks.json',
                method: 'GET'
            }),
        ...
        var dependencyStore = new Ext.data.Store({
            //autoLoad: true,
            proxy: new Ext.data.HttpProxy({
                url: 'lib/ext-gantt/dependencies.xml',
                //url : 'lib/ext-gantt/dependencies.json',
                method: 'GET'
            }),
            reader: new Ext.data.XmlReader({
                // records will have a 'Task' tag
        ...
        var gantt = new Sch.gantt.GanttPanel({
            height: window.innerHeight-(window.innerHeight*0.04),
            width: window.innerWidth-(window.innerWidth*0.127),
            //anchor:'100 %',
            ...
            eventRenderer: function (taskRecord) {
				document.getElementById('ext-gen66').style.display = 'none';
                return {
            ...
            tooltipTpl: new Ext.XTemplate(
                ...
                '<table class="taskTip">',
                    '<tr><td>Inicio:</td> <td align="right">{[values.StartDate.format("y-m-d")]}</td></tr>',
                    '<tr><td>Fin:</td> <td align="right">{[values.EndDate.format("y-m-d")]}</td></tr>',
                    '<tr><td>Progreso:</td><td align="right">{PercentDone}%</td></tr>',
                '</table>'
            ).compile(),
        ...
        colSlider.on({
            change: function (s, v) {
                gantt.updateTimeColumnHeaderWidths(v);
            },
            changecomplete: function (s, v) {
                gantt.setTimeColumnWidth(v);
            }
        });
