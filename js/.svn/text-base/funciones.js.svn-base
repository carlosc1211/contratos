...
/*******************************************************************************************/
	...
	// Validacion con formato alfanumerico incluyendo la Ò—
		validosEsp : function(){
			var re = /^([-_.,A-Z0-9\/:·¡È…ÌÕÛ”˙⁄Ò—#%&+*;\)\( ])+$/i;
			return function(v) {
				return re.test(v);
			}
		}(),
		validosEspText : 'Este campo solo admite caracteres alfanumericos y simbolos estandares...',
		validosEspMask : /([-_.,A-Z0-9\/:·¡È…ÌÕÛ”˙⁄Ò—#%&+*;\)\( ])/i,
...
/****************************************************************************/
function crearTab(id, titulo, contenido){
	tab = new Ext.Panel({
						id: id,
						title: titulo,
						//contentEl:div_carga,
						...
						border: false,
						layout: 'ux.center', 
						plugins: getPlugin(contenido, 'show')
	});
	Ext.getCmp('tabs').add(tab);
	tab.show();
	return;
}

/****************************************************************************/
function cargarTab(id, titulo, contenido, tabPanel, subTab)
	{
		//Variable para saber si el tab esta abierto
		var open =  !Ext.getCmp('tabs').getItem(id);
		if(subTab)
			prodTab = subTab-1;
		if(open){
			crearTab(id, titulo, contenido);
		}
		else{
			tab =  Ext.getCmp('tabs').getItem(id); //paneldetabs.getItem(nombre);
			tab.show();	
			if(tabPanel)
				Ext.getCmp(tabPanel).setActiveTab(prodTab);
		}
		return;
}

/**********************************************************/
var getPlugin = function(remoteUrl, evento){
		//alert( Ext.getCmp("tx_pozo").getValue());
		return  new Ext.ux.Plugin.RemoteComponent({
							 url: remoteUrl //'trabPozos.php?pozo='+'FUL 21%'
							,method: 'GET'
							,loadOn: evento
							,mask: 'tabs'
							,maskConfig: {
