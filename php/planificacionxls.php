...
//******************************CONSTRUIMOS LA HOJA*********************************************
	...
	$hoja->write(0,0,utf8_decode("GERENCIA DE CONTRATACIÓN ''PROCESOS PLANIFICADOS''"),$N_grande);
	$hoja->mergeCells(0,0,3,12);
	foreach($header as $key => $value){
		$hoja->write(6,$key,utf8_decode($value),$C_tope);
		}
//*************************COLOCAMOS EL ANCHO DE LAS COLUMNAS***********************************
	...
	$hoja->setColumn($columna['nu_anio'],$columna['nu_anio'],6);
	$hoja->setColumn($columna['tx_usuario'],$columna['tx_supervisor'],30);
	$hoja->setColumn($columna['tx_proyecto'],$columna['tx_proyecto'],50);
	$hoja->setColumn($columna['tx_ubicacion'],$columna['tx_ubicacion'],25);
	$hoja->setColumn($columna['tx_organizacion'],$columna['tx_organizacion'],30);
	$hoja->setColumn($columna['tx_ubicacion_etc'],$columna['tx_ubicacion_etc'],20);
	$hoja->setColumn($columna['tx_lugar_ejecucion'],$columna['tx_lugar_ejecucion'],25);
	$hoja->setColumn($columna['nu_orden_interna'],$columna['nu_orden_interna'],13);
	$hoja->setColumn($columna['nu_renglon'],$columna['nu_renglon'],21);
	$hoja->setColumn($columna['tx_prioridad'],$columna['tx_prioridad'],9);
	$hoja->setColumn($columna['nu_plazo_ejec'],$columna['nu_plazo_ejec'],18);
	$hoja->setColumn($columna['tx_tipo_presupuesto'],$columna['tx_tipo_presupuesto'],15);
	$hoja->setColumn($columna['tx_tipo_act'],$columna['tx_tipo_act'],25);
	$hoja->setColumn($columna['fe_especificaciones'],$columna['fe_especificaciones'],22);
	$hoja->setColumn($columna['fe_inicio_estimada_p'],$columna['fe_inicio_estimada_p'],33);
	$hoja->setColumn($columna['fe_inicio_estimada_a'],$columna['fe_inicio_estimada_a'],30);
	$hoja->setColumn($columna['tx_naturaleza'],$columna['tx_naturaleza'],26);
	$hoja->setColumn($columna['tx_estrategia_adj'],$columna['tx_estrategia_adj'],22);
	$hoja->setColumn($columna['nu_contratos_asociados'],$columna['nu_contratos_asociados'],10);
	$hoja->setColumn($columna['tx_tipo_contrato'],$columna['tx_tipo_contrato'],65);
	$hoja->setColumn($columna['tx_regimen_lab'],$columna['tx_regimen_lab'],14);
	$hoja->setColumn($columna['tx_modalidad'],$columna['tx_modalidad'],35);
	$hoja->setColumn($columna['tx_mecanismo_verif'],$columna['tx_mecanismo_verif'],25);
	$hoja->setColumn($columna['tx_rango'],$columna['tx_rango'],20);
	$hoja->setColumn($columna['monto_max'],$columna['monto_max'],14);
...
//**************************************CONECTAMOS A LA BD***************************************************
	...
	$result=mysql_query("
	SELECT 
		(@rownum:=@rownum+1) AS rownum,c001t_usuario.*, u.*, tu.*, 	tc.*, ea.*, o.*, na.*, mo.*,rl.*,mv.*,ra.*,tp.*,tac.*,pri.*,
        CONCAT(tx_rango,' (',ut_minimo,' - ' ,ut_maximo,' UT)') AS tx_rango	FROM  (SELECT @rownum:=0) r, c003t_proyecto
